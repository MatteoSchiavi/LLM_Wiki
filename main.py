"""
╔══════════════════════════════════════════════════════════╗
║              SECONDO CERVELLO  —  main.py                ║
║   One process · One port · OCR + Wiki + UI               ║
╚══════════════════════════════════════════════════════════╝
python main.py  →  http://localhost:7337
"""
...
logging.basicConfig(
    ...
    datefmt="%H:%M:%S",
    handlers=[logging.StreamHandler(), logging.FileHandler("cervello.log", encoding="utf-8")],
)
...
for d in [RAW, WIKI_DIR / "entities", WIKI_DIR / "concepts", WIKI_DIR / "sources",
          UPLOADS_TMP, DB_PATH.parent, TMPL_DIR]:
    d.mkdir(parents=True, exist_ok=True)

# ─── Global state ─────────────────────────────────────────────────────────────
wiki   = WikiEngine(VAULT, CFG)
search = BM25Search(WIKI_DIR)
...
async def lifespan(app: FastAPI):
    ...
    log.info("Ready → http://localhost:7337")
    yield


app = FastAPI(title="Secondo Cervello", lifespan=lifespan)
...
def wiki_tree():
    def _tree(path: Path, base: Path) -> dict:
        if path.is_file():
            return {"name": path.name, "path": str(path.relative_to(base)),
                    "type": "file"}
        children = sorted(
            [_tree(c, base) for c in path.iterdir() if not c.name.startswith(".")],
    ...
    return _tree(WIKI_DIR, WIKI_DIR)


@app.get("/api/wiki/page")
def wiki_page(path: str):
    p = (WIKI_DIR / path).resolve()
    # Security: ensure path stays within WIKI_DIR
    if not str(p).startswith(str(WIKI_DIR.resolve())):
...
async def query(request: Request):
    ...
    results = search.search(question, k=6)
    context_pages = []
    for r in results:
        p = WIKI_DIR / r["path"]
        if p.exists():
...
def stats():
    ...
    entities = count_dir(WIKI_DIR / "entities")
    concepts = count_dir(WIKI_DIR / "concepts")
    sources  = count_dir(WIKI_DIR / "sources")
    ...
    return {
        "wiki":  {"entities": entities, "concepts": concepts, "sources": sources, "total": total},
...
async def health():
    ...
    try:
        async with httpx.AsyncClient() as c:
            r = await c.get(CFG["ollama_url"] + "/api/tags", timeout=3.0)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("main:app", host="0.0.0.0", port=7337, reload=False)
